...
%% Blind submission
\usepackage[submission]{probml}

%% For ArXiv, etc.
% \usepackage[preprint]{probml}

%% Accepted, camera-ready
% \usepackage{probml}

\ShortHeadings{ProbML Style Guide}
%% ------------------------------------------------------------
...
\affil[$\dagger$]{Correspondence to \url{email@example.com}}
...
\begin{wrapfigure}{r}{0.4\textwidth}
  ...
  \includegraphics[width=0.35\textwidth]{images/nodes}
...
Figures should go in the \texttt{figure} environment.
If you want to scale the
image, it's better to use a fraction of the line width rather
than an explicit length. For example, see \cref{fig:nodes}.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.5\linewidth]{images/nodes}
  % \caption{Example Image asdasdsada dsad a sdasdasd asjdhaksjdh kajshkjdhajsk hdkjashdjkahs jkdhasjkdh akjshdkjas hdkjash dkjashdkj ahs kdjhaskdjaskd hkjasha}
...
Sections in appendices are referenced using \verb|\appendix| and \verb|\cref|. For example, \cref{apd:first} or \cref{apd:first,apd:second}.
...
You can use the \texttt{proof} environment
...
\begin{definition} \label{def:set}
  The set $A$ has three members: $a$, $b$, and $c$.
\end{definition}
...
Citing the definition: \cref{def:set}. Also, \cref{prop:set}. \Cref{cor:set} is another one.


\subsection{Full-Width Tables}
\label{sec:fullwidth-tables}

For tables that need to span the full width of the page or contain many columns, you can use the \texttt{table*} environment. Here's an example using booktabs for professional formatting:

\begin{table*}[t]
  \centering

  \caption{Full-Width Table Example: Performance Comparison Across Different Methods}
  \label{tab:fullwidth-example}

  \begin{tabular}{lccccc}
    \toprule
    \textbf{Method}   & \textbf{Dataset 1} & \textbf{Dataset 2} & \textbf{Dataset 3} & \textbf{Dataset 4} & \textbf{Runtime (s)} \\
    \midrule
    Baseline Method   & 0.823              & 0.756              & 0.734              & 0.812              & 125.4                \\
    Improved Method A & 0.847              & 0.789              & 0.768              & 0.834              & 98.7                 \\
    Improved Method B & 0.856              & 0.801              & 0.782              & 0.845              & 87.3                 \\
    Our Method        & \textbf{0.871}     & \textbf{0.823}     & \textbf{0.796}     & \textbf{0.862}     & \textbf{73.2}        \\
    \bottomrule
  \end{tabular}
\end{table*}
...
\begin{wraptable}{r}{0.45\textwidth}
  ...
  \begin{tabular}{lcc}
    ...
    Method A        & 85.3\%            & 12.4s          \\
    Method B        & 87.1\%            & 15.7s          \\
    Our Method      & \textbf{89.6\%}   & \textbf{10.2s} \\
    \bottomrule
  \end{tabular}
\end{wraptable}

Sometimes you want a table to be embedded within the text flow. The \texttt{wraptable} environment allows text to wrap around the table.

This paragraph demonstrates how text flows around the wrapped table. The table appears on the right side of the text, and the content naturally wraps around it. This is particularly useful for smaller tables that contain summary information or key results that you want to highlight without breaking the flow of the main text.

You can position the table on the left using \verb|{l}| or on the right using \verb|{r}| as the first parameter to \texttt{wraptable}. The second parameter controls the width allocated to the table.

The wrapped table maintains proper spacing and formatting while allowing the text to flow naturally. This technique is especially effective in documents where you want to present data without interrupting the narrative flow. Notice how this paragraph continues to wrap around the table, demonstrating the natural text flow that can be achieved with this approach.

\blindtext[1]

The \texttt{table*} environment is ideal for comprehensive comparison tables, while \texttt{wraptable} works well for smaller summary tables that complement the surrounding text.
`
...
Please comment this section for the double-blind submission.
